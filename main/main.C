...
int main(int argc, char *argv[])
{
   ...
   ProcessArgs(argc, argv, depth, fitfunc, ptm, ptM, ym, yM, nameFile, verbose, mode, canvas);
   ...
   if (mode == 0) {                                                // if mode is set to zero, start query for the fit
      ...
      RooFitResult *fitresult = fitRoo(h, mode, fitfunc, depth, ptm, ptM, ym, yM, nameFile, verbose, canvas);
   }

   if (mode == 1) // if mode is set to one, start query for the differential cross section calculation
   {
      PlotDiffCrossSection(df, ym, yM, depth, canvas);
   }
