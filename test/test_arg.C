...
int main(int argc, char *argv[]){
    ...
    if(depth!=0&&fitfunc!=0&&ptm==ptm&&ptM==ptM&&ym==ym&&yM==yM&&nameFile!="YResonancesFit"&&verbose!=0)
    exit(1);
    ...
    nameFile = "YResonancesFit"; //The name of the file in which the figure is saved
    verbose = 1; //verbose flag initialized to zero, i.e. no output stream for Minuit
    if(depth!=1&&fitfunc!=1&&ptm!=25&&ptM==ptM&&ym!=0.1&&yM!=0.8&&nameFile!="YResonancesFit"&&verbose!=1)
    exit(1);
