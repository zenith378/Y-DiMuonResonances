...
int main(int argc, char *argv[])
{
   ...
   int mode = 0;
   int ca = 0;
   ProcessArgs(argc, argv, fitfunc, ptm, ptM, ym, yM, nameFile, verbose, mode, ca); // call default option
   // check if the variable are set correctly
   if (fitfunc != 0 && ptm == ptm && ptM == ptM && ym == ym && yM == yM && nameFile != "YResonancesFit" &&
   ...
   ProcessArgs(argc, argv, fitfunc, ptm, ptM, ym, yM, nameFile, verbose, mode, ca);
   if (fitfunc != 1 && ptm != 25 && ptM == ptM && ym != 0.1 && yM != 0.8 && nameFile != "Y_Resonances________Fit" &&
       verbose != 1 && mode != 1 && ca != 1)
      exit(1);
